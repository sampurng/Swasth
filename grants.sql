...
EXCEPTION
    WHEN E_ROLES_ABSENT THEN
        ...
        BEGIN
            SELECT COUNT(*) INTO v_role_exists FROM DBA_ROLES WHERE ROLE = 'SWASTH_ADMIN';
            
            IF v_role_exists = 0 THEN
                EXECUTE IMMEDIATE ('CREATE ROLE SWASTH_ADMIN');
                DBMS_OUTPUT.PUT_LINE('CREATED SWASTH_ADMIN ROLE');
            ELSE
                DBMS_OUTPUT.PUT_LINE('SWASTH_ADMIN ROLE ALREADY EXISTS');
            END IF;
        ...
        BEGIN
            SELECT COUNT(*) INTO v_role_exists FROM DBA_ROLES WHERE ROLE = 'SWASTH_EVENT_MANAGER';
            
            IF v_role_exists = 0 THEN
                EXECUTE IMMEDIATE ('CREATE ROLE SWASTH_EVENT_MANAGER');
                DBMS_OUTPUT.PUT_LINE('CREATED SWASTH_EVENT_MANAGER ROLE');
            ELSE
                DBMS_OUTPUT.PUT_LINE('SWASTH_EVENT_MANAGER ROLE ALREADY EXISTS');
            END IF;
            
            -- Granting permissions for SWASTH_EVENT_MANAGER role
            EXECUTE IMMEDIATE 'GRANT SELECT ON user_details TO SWASTH_EVENT_MANAGER';
            EXECUTE IMMEDIATE 'GRANT SELECT ON sleep_details TO SWASTH_EVENT_MANAGER';
            EXECUTE IMMEDIATE 'GRANT SELECT ON sleep_metrics TO SWASTH_EVENT_MANAGER';
            EXECUTE IMMEDIATE 'GRANT SELECT ON exercise_details TO SWASTH_EVENT_MANAGER';
            EXECUTE IMMEDIATE 'GRANT SELECT ON exercise_metrics TO SWASTH_EVENT_MANAGER';
            EXECUTE IMMEDIATE 'GRANT SELECT ON health_details TO SWASTH_EVENT_MANAGER';
            EXECUTE IMMEDIATE 'GRANT SELECT ON body_composition TO SWASTH_EVENT_MANAGER';
        END;

        BEGIN
            SELECT COUNT(*) INTO v_role_exists FROM DBA_ROLES WHERE ROLE = 'SWASTH_CLIENT';
            
            IF v_role_exists = 0 THEN
                EXECUTE IMMEDIATE ('CREATE ROLE SWASTH_CLIENT');
                DBMS_OUTPUT.PUT_LINE('CREATED SWASTH_CLIENT ROLE');
            ELSE
                DBMS_OUTPUT.PUT_LINE('SWASTH_CLIENT ROLE ALREADY EXISTS');
            END IF;
            
            -- Granting permissions for SWASTH_CLIENT role
            EXECUTE IMMEDIATE 'GRANT SELECT ON user_details TO SWASTH_CLIENT';
            EXECUTE IMMEDIATE 'GRANT SELECT ON sleep_details TO SWASTH_CLIENT';
            EXECUTE IMMEDIATE 'GRANT SELECT ON sleep_metrics TO SWASTH_CLIENT';
            EXECUTE IMMEDIATE 'GRANT SELECT ON exercise_details TO SWASTH_CLIENT';
            EXECUTE IMMEDIATE 'GRANT SELECT ON exercise_metrics TO SWASTH_CLIENT';
            EXECUTE IMMEDIATE 'GRANT SELECT ON health_details TO SWASTH_CLIENT';
            EXECUTE IMMEDIATE 'GRANT SELECT ON body_composition TO SWASTH_CLIENT';
        END;
