...
SELECT D.DNAME, E.SumSal/E.EmpCount as avg FROM 
    (
        SELECT SUM(SAL) as SumSal, COUNT(EMPNO) as EmpCount, DEPTNO FROM EMP GROUP BY DEPTNO
    ) 
...
(SELECT TO_CHAR(A.HIREDATE, 'YYYY') as Year, To_CHAR(A.HIREDATE, 'MM') as Month FROM  (SELECT HIREDATE FROM EMP WHERE ENAME = 'SCOTT') A) AB
...
FROM 
    (SELECT Count(*) over () as tc, COUNT(EMPNO) as Total_Employees, DEPTNO, RANK() over (ORDER BY COUNT(EMPNO)) as Rank 
